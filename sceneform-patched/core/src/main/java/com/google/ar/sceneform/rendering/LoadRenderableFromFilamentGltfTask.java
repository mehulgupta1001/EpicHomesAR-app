...
public class LoadRenderableFromFilamentGltfTask<T extends Renderable> {
  ...
  LoadRenderableFromFilamentGltfTask(
    ...
    // Use Filament's ResourceLoader (4-arg constructor: normalizeSkinningWeights, etc.)
    // so embedded GLB textures load correctly. Creating a new ResourceLoader(engine) here
    // can leave materials/textures unloaded (white models).
    this.renderableData.resourceLoader = Filament.INSTANCE.getResourceLoader();
    ...
    return CompletableFuture.supplyAsync(
        ...
        .thenApplyAsync(
            gltfByteBuffer -> {
              ...
              this.renderableData.gltfBytes = gltfByteBuffer;
              // Use a direct ByteBuffer so native gltfio can access embedded texture data
              // (bufferView images). Heap buffers may not give native a valid pointer.
              ByteBuffer direct = ByteBuffer.allocateDirect(gltfByteBuffer.length);
              direct.put(gltfByteBuffer);
              direct.flip();
              this.renderableData.gltfByteBuffer = direct;
              return renderable;
            },
