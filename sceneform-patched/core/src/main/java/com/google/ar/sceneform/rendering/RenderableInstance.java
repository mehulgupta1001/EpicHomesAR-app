...
public class RenderableInstance implements AnimatableModel {
  ...
  private final java.util.ArrayList<com.google.android.filament.Texture> instanceTextures =
          new java.util.ArrayList<>();
  private boolean pendingTextureBind = true;
  private boolean materialFactorsApplied = false;
    ...
    void createFilamentAssetModelInstance() {
        if (renderable.getRenderableData() instanceof RenderableInternalFilamentAssetData) {
            ...
            // Keep engine referenced for native Filament calls in this scope.
            if (engine == null) {
                throw new IllegalStateException("Filament engine not initialized");
            }

            ByteBuffer glbBuffer =
                    GltfBufferUtils.prepareForNativeLoad(
                            renderableData, renderableData.gltfByteBuffer);
            ...
            FilamentAsset createdAsset =
                    renderableData.isGltfBinary
                            ? loader.createAssetFromBinary(glbBuffer)
                            : loader.createAssetFromJson(glbBuffer);

            if (createdAsset == null) {
                throw new IllegalStateException("Failed to load gltf");
            }

            String[] resourceUris = createdAsset.getResourceUris();
            Log.d(
                    TAG,
                    "GLB loaded: entities="
                            + createdAsset.getEntities().length
                            + " resourceUris="
                            + resourceUris.length
                            + " asyncLoad="
                            + renderable.asyncLoadEnabled);
            ...
            for (int entity : createdAsset.getEntities()) {
                ...
                MaterialInstance materialInstance =
                        renderableManager.getMaterialInstanceAt(renderableInstance, 0);
                materialNames.add(materialInstance.getName());

                MaterialInternalDataGltfImpl materialData =
                        new MaterialInternalDataGltfImpl(materialInstance.getMaterial());
            ...
            // Solid colors immediately; PNG upload deferred to prepareForDraw (needs GL context).
            applyEmbeddedGltfMaterials(createdAsset, renderableManager);
    ...
    private void applyEmbeddedGltfMaterials(FilamentAsset asset, RenderableManager renderableManager) {
        if (materialFactorsApplied) {
            return;
        }
        if (!(renderable.getRenderableData() instanceof RenderableInternalFilamentAssetData)) {
            return;
        ...
        RenderableInternalFilamentAssetData renderableData =
                (RenderableInternalFilamentAssetData) renderable.getRenderableData();
        byte[] glbBytes = renderableData.gltfBytes;
        if (glbBytes == null) {
            glbBytes = GltfEmbeddedTextureBinder.bytesFromBuffer(renderableData.gltfByteBuffer);
        }
        if (glbBytes == null) {
            Log.w(TAG, "No GLB bytes available for material factor binding");
            return;
        }
        GltfEmbeddedTextureBinder.applyMaterialFactors(glbBytes, asset, renderableManager);
        materialFactorsApplied = true;
    }

    private void applyEmbeddedGltfTextures(FilamentAsset asset) {
        if (!(renderable.getRenderableData() instanceof RenderableInternalFilamentAssetData)) {
            return;
        }
        RenderableInternalFilamentAssetData renderableData =
        ...
        if (glbBytes == null) {
            Log.w(TAG, "No GLB bytes available for texture binding");
            return;
        }
        Engine engine = EngineInstance.getEngine().getFilamentEngine();
        RenderableManager renderableManager = EngineInstance.getEngine().getRenderableManager();
        GltfEmbeddedTextureBinder.applyMaterialTextures(
                glbBytes, asset, engine, renderableManager, instanceTextures);
    }
...
//        }
    ...
    public void prepareForDraw() {
        ...
        if (pendingTextureBind && filamentAsset != null && attachedRenderer != null) {
            pendingTextureBind = false;
            applyEmbeddedGltfTextures(filamentAsset);
        }
    ...
    public void destroy() {
        ...
        if (!instanceTextures.isEmpty()) {
            Engine engine = EngineInstance.getEngine().getFilamentEngine();
            for (com.google.android.filament.Texture texture : instanceTextures) {
